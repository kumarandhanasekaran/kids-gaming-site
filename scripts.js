...
function renderGames(games) {
    ...
    const gameCards = games.map(game => {
        ...
        const features = game.features ? game.features.slice(0, 3).map(feature => 
        ...
        return `
            <div class="game-card" onclick="openGame('${game.url}')" 
                 ...
                 data-category="${game.category}" data-name="${game.name.toLowerCase()}">
                <img src="${game.thumb || './assets/images/placeholder-game.png'}" 
                ...
                <div class="game-content">
                    ...
                    <div class="game-info">
                        <small>Ages: ${game.ageGroup} | Time: ${game.playTime}</small>
                    </div>
                </div>
            </div>
        `;
...
function populateCategoryFilter() {
    ...
    const categories = gamesData.categories.map(cat => 
        `<option value="${cat.name}">${cat.icon} ${cat.name}</option>`
    ).join('');
    
    filterSelect.innerHTML = '<option value="">All Categories</option>' + categories;
}
...
function setupFallbackContent() {
    ...
    const gamesGrid = document.getElementById('games-grid');
    if (gamesGrid) {
        const gameCards = fallbackGames.map(game => `
            <div class="game-card" onclick="openGame('${game.url}')" 
                 role="button" tabindex="0" aria-label="Play ${game.name}">
...
function searchGames(query) {
    ...
    if (!resultMessage) {
        ...
        gamesSection.insertBefore(resultMessage, document.getElementById('games-grid'));
    }
